...
class LoginTest(APITestCase):
    def test_login_superuser(self):
        ...
        created_superuser = self.client.post('/api/accounts/', superuser_sign_in_data, format='json')
        ...
        response = self.client.post('/api/login/', sign_up_data, format='json')

        self.assertEqual(response.status_code, 200)
        self.assertIn('token', response.json.ke)
